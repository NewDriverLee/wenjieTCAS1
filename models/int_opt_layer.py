...
class QuantOPTAttention(nn.Module):
    ...
    def forward(
        ...
        #if(self.debug==1):
        #    print("QuantOPTAttention")
        #    print(f"self.use_weight_quant = {self.use_weight_quant},self.use_act_quant = {self.use_act_quant}")

        # get query proj
        #if(self.debug==1):
        #    print(f"hidden_states 1: {hidden_states.shape}, self.q_proj.weight.shape={self.q_proj.weight.shape}")
        #    print(hidden_states)
        ...
        #if(self.debug==1):
        #    print(f"query_states 2:  {query_states.shape}")
        #    print(query_states)
        
        
        #if(self.debug==1):
        #    self.qkt_matmul.debug=1
        
        #if(self.debug==1):
        #    print(f"self.qkt_matmul.use_act_quant={self.qkt_matmul.use_act_quant},self.qkt_matmul.x1_quantizer={self.qkt_matmul.x1_quantizer}, self.qkt_matmul.x2_quantizer={self.qkt_matmul.x2_quantizer}")
        #    self.qkt_matmul.x1_quantizer.debug=1
        #    self.qkt_matmul.x2_quantizer.debug=1
        
        #if(self.debug==1):
        #    print(f"self.pv_matmul.use_act_quant={self.pv_matmul.use_act_quant},self.pv_matmul.x1_quantizer={self.pv_matmul.x1_quantizer}, self.pv_matmul.x2_quantizer={self.pv_matmul.x2_quantizer}, self.pv_matmul.x1_quantizer.n_bit={self.pv_matmul.x1_quantizer.n_bits},self.pv_matmul.x2_quantizer.n_bit={self.pv_matmul.x2_quantizer.n_bits}")
        #    #self.pv_matmul.x1_quantizer.debug=1
        #    #self.pv_matmul.x2_quantizer.debug=1
        
        #if(self.debug==1):
        #    print(f"self.qkt_matmul.x1_quantizer.scale: {self.qkt_matmul.x1_quantizer.scale.shape}")
        #    print(self.qkt_matmul.x1_quantizer.scale)
        #    print(f"query_states > self.qkt_matmul.x1_quantizer.scale: {(query_states > self.qkt_matmul.x1_quantizer.scale.long()).shape}")
        #    print((query_states > self.qkt_matmul.x1_quantizer.scale).long())
        
        query_states = self.qkt_matmul.quant_x1(query_states)
        
        #if(self.debug==1):
        #    print(f"query_states 3:   {query_states.shape}")
        #    print(query_states)
        ...
        if(self.debug==1):
            self.qkt_matmul.x1_quantizer.debug=0
            self.qkt_matmul.x2_quantizer.debug=0
        ...
        #if(self.debug==1):
        #    print(f"self.out_proj.disable_input_quant={self.out_proj.disable_input_quant},self.out_proj.act_quantizer.n_bits={self.out_proj.act_quantizer.n_bits}")
        
        attn_output = attn_output.reshape(bsz, tgt_len, self.embed_dim)
        attn_output = self.out_proj(attn_output)

        if(self.debug==1):
            self.pv_matmul.x1_quantizer.debug=0
            self.pv_matmul.x2_quantizer.debug=0
