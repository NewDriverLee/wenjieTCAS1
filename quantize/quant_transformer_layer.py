...
def quant_layer(qlayer, args, outs, inps, attention_mask, dev):
    ...
    for name, quantizer in a_quantizers.items():
        #print(f"name={name},quantizer={quantizer},quantizer.metric={quantizer.metric},quantizer.symmetric={quantizer.symmetric},quantizer.cluster_dim={quantizer.cluster_dim}")
        quantizer.set_calibration_mode()
        
    # calibration for the quantizers
    ...
    for name, quantizer in a_quantizers.items():
        #print(f"name={name},quantizer={quantizer},")
        #if(quantizer.cached_xmax is not None):
        #    print(f"quantizer.cached_xmax: {quantizer.cached_xmax.shape}")
        #    print(quantizer.cached_xmax)
        #    print(f"quantizer.cached_xmin: {quantizer.cached_xmin.shape}")
        #    print(quantizer.cached_xmin)
        #else:
        #    print(f"quantizer.cached_xmax: {quantizer.cached_xmax}")
        #    print(f"quantizer.cached_xmin: {quantizer.cached_xmin}")
