...
def layer_omax_hook(m, i, o):
    ...
    #print(f"name={name}, o.ndim={o.ndim}")
    #if(name=='self_attn_layer_norm'):
    #    print(f"o.argmax ({o.shape}): ({int(torch.argmax(o)/5120)},{torch.argmax(o)%5120}), o.argmin: ({int(torch.argmin(o)/5120)},{torch.argmin(o)%5120}), torch.max(o)={torch.max(o)}, torch.max(o)={torch.min(o)}")
    #    #for i in range(o.shape[1]):
    #    #    print(f"{i}: {o[0][i][torch.argmax(o)%5120]}")
...
def tensor_calc_reorder_index(xmax, xmin, n_clusters, n_heads=None):
    ...
    for i, data in enumerate(npdata):
        ...
        #if(data.shape[0]==5120 and data.shape[1]==2):
        #    for j in range(0,5120):
        #        print(f"{j}: {data[j][0]}, {data[j][1]}")
        ...
        #if(data.shape[0]==5120 and data.shape[1]==2):
        #    print(f"kmeans.labels_: {len(kmeans.labels_)}")
        #    print(kmeans.labels_)
        #    if(len(kmeans.labels_)>902):
        #        print(f"kmeans.labels_[902]: {kmeans.labels_[902]}")
        #    
        #    for ii in range(data.shape[0]):
        #        if(kmeans.labels_[ii]==2):
        #            tmp_channel=ii
        #            break
        #            #print(f"kmeans.labels_[{ii}]==2")
        #    print(f"tmp_channel={tmp_channel}, npdata[0][{tmp_channel}]=({npdata[0][tmp_channel][0]},{npdata[0][tmp_channel][1]})")
